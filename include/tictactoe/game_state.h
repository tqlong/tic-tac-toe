...
public:
    ...
    void setPlayScreen(PlayScreen playScreen_) { playScreen = playScreen_; }

    PlayMode getPlayMode() const { return playMode; }
    void setPlayMode(PlayMode playMode_) { playMode = playMode_; }

    // get position i and j from the given index
    char getPlay(unsigned int i, unsigned int j) const;
...
private:
    ...
    // Play the game on the given position
    State play(State s, unsigned int i, unsigned int j, char player);

    void setPlay(unsigned int i, unsigned int j, char player);
    void setPlay(int i, int j, char player);
    ...
    std::pair<unsigned int, unsigned int> getComputerPlay(State s);
